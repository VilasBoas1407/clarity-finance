      ...
      <nav className="fixed top-0 left-0 right-0 z-50 border-b border-sidebar-border bg-sidebar/90 backdrop-blur-md">
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-8">
            <a href="#features" className="text-sm text-sidebar-foreground hover:text-sidebar-accent-foreground transition-colors">
              Funcionalidades
            </a>
            <a href="#pricing" className="text-sm text-sidebar-foreground hover:text-sidebar-accent-foreground transition-colors">
              Preços
            </a>
            <a href="#about" className="text-sm text-sidebar-foreground hover:text-sidebar-accent-foreground transition-colors">
              Sobre
            </a>
          </div>
          <div className="flex items-center gap-3">
            <Button variant="ghost" className="text-sidebar-foreground hover:text-sidebar-accent-foreground" asChild>
        ...
        <div className="max-w-7xl mx-auto">
          <div className="max-w-3xl mx-auto text-center mb-16">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
              ...
              <Button size="lg" variant="outline" className="border-sidebar-border text-sidebar-accent-foreground hover:bg-sidebar-accent px-8 h-12 text-base">
                Ver demonstração
              </Button>
            </div>
          </div>
          ...
          <div className="max-w-4xl mx-auto">
            <div className="rounded-xl border border-sidebar-border bg-sidebar-accent p-6 shadow-lg">
              <div className="grid grid-cols-3 gap-4 mb-6">
                {[
                  { label: "Gasto mensal", value: "R$ 4.280", icon: TrendingUp, change: "-5%" },
                  { label: "Recorrentes", value: "R$ 1.890", icon: RefreshCw, change: "+2%" },
                  { label: "Saldo livre", value: "R$ 3.720", icon: Shield, change: "+12%" },
                ].map((kpi) => (
                  <div key={kpi.label} className="rounded-lg border border-sidebar-border bg-sidebar p-4">
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-xs text-sidebar-foreground">{kpi.label}</span>
                      <kpi.icon className="w-4 h-4 text-primary" />
                    </div>
                    <p className="text-xl font-bold text-sidebar-accent-foreground">{kpi.value}</p>
                ...
                <div className="rounded-lg border border-sidebar-border bg-sidebar p-4 h-32 flex items-center justify-center">
                  <div className="relative w-24 h-24">
                    <svg viewBox="0 0 100 100" className="w-full h-full -rotate-90">
                      <circle cx="50" cy="50" r="40" fill="none" stroke="hsl(var(--sidebar-border))" strokeWidth="12" />
                      <circle cx="50" cy="50" r="40" fill="none" stroke="hsl(var(--primary))" strokeWidth="12" strokeDasharray="251.2" strokeDashoffset="75" strokeLinecap="round" />
                      <circle cx="50" cy="50" r="40" fill="none" stroke="hsl(var(--chart-2))" strokeWidth="12" strokeDasharray="251.2" strokeDashoffset="188" strokeLinecap="round" />
                    </svg>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
            {stats.map((stat) => (
              <div key={stat.label} className="text-center">
                <p className="text-3xl font-bold text-sidebar-accent-foreground">{stat.value}</p>
                <p className="text-sm text-sidebar-foreground mt-1">{stat.label}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-16">
            ...
            <p className="text-sidebar-foreground max-w-xl mx-auto">
              Ferramentas poderosas e simples que transformam dados financeiros em decisões claras.
            </p>
          </div>
          <div className="grid md:grid-cols-3 gap-6">
            {features.map((feature) => (
              <Card key={feature.title} className="bg-sidebar-accent border-sidebar-border hover:border-primary/30 transition-colors group">
      ...
      <section id="about" className="py-20 px-6 border-t border-sidebar-border">
        <div className="max-w-3xl mx-auto text-center">
          ...
          <p className="text-sidebar-foreground leading-relaxed mb-6">
            O Clarity Finance nasceu da frustração com planilhas complexas e apps confusos.
            Nosso objetivo é dar clareza financeira real — com uma experiência bonita, rápida e que cresce com você.
          </p>
      ...
      <section id="pricing" className="py-24 px-6">
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-16">
            <h2 className="text-3xl md:text-4xl font-bold mb-4">
              Planos para cada{" "}
              <span className="gradient-text">momento</span>
            </h2>
            ...
            {plans.map((plan) => (
              ...
              >
                ...
                <CardContent className="p-6 pt-8">
                  ...
                  <p className="text-sm text-sidebar-foreground mb-4">{plan.description}</p>
                  <div className="mb-6">
                    <span className="text-4xl font-bold text-sidebar-accent-foreground">{plan.price}</span>
                    <span className="text-sidebar-foreground">{plan.period}</span>
                  </div>
                  <Button
                  ...
                  <ul className="space-y-3">
                    {plan.features.map((feature) => (
                      <li key={feature} className="flex items-start gap-2 text-sm text-sidebar-foreground">
      ...
      <footer className="border-t border-sidebar-border py-12 px-6">
        <div className="max-w-5xl mx-auto flex flex-col md:flex-row items-center justify-between gap-6">
          <div className="flex items-center gap-2">
            ...
            <span className="font-semibold text-sidebar-accent-foreground">Clarity Finance</span>
            <span className="text-xs text-sidebar-foreground ml-2">Clareza para suas finanças</span>
          </div>
          <div className="flex items-center gap-6 text-sm text-sidebar-foreground">
            <a href="#" className="hover:text-sidebar-accent-foreground transition-colors">Termos</a>
            <a href="#" className="hover:text-sidebar-accent-foreground transition-colors">Privacidade</a>
            <a href="#" className="hover:text-sidebar-accent-foreground transition-colors">Contato</a>
          </div>
